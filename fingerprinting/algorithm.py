...
from .signature_format_old import DecodedMessage, FrequencyPeak, RawSignatureHeader, FrequencyBand
...
class SignatureGenerator:
    ...
    def __init__(self):
        ...
        self.input_pending_processing = [] # Signed 16-bits, 16 KHz mono samples to be processed
        
        self.samples_processed = 0 # Number of samples processed out of "self.input_pending_processing"
        
        # Used when processing input:
        
        self.ring_buffer_of_samples = RingBuffer(buffer_size = 2048, default_value = 0)
        
        self.fft_outputs = RingBuffer(buffer_size = 256, default_value = [0. * 1025]) # Lists of 1025 floats, premultiplied with a Hanning function before being passed through FFT, computed from the ring buffer every new 128 samples
    ...
    def get_next_signature(self):
        ...
        self.ring_buffer_of_samples  = RingBuffer(buffer_size = 2048, default_value = 0)
        self.fft_outputs = RingBuffer(buffer_size = 256, default_value = [0. * 1025])
        self.spread_ffts_output = RingBuffer(buffer_size = 256, default_value = [0] * 1025)
    ...
    def do_peak_recognition(self):
        ...
        for bin_position in range(10, 1015):
            ...
            if (fft_minus_46[bin_position] >= 1 / 64 and
                ...
                for neighbor_offset in list(range(-10, -3, 3)) + [-3, 1] + list(range(2, 9, 3)):
                ...
                if fft_minus_46[bin_position] > max_neighbor_in_fft_minus_49:
                    ...
                    for other_offset in [-53, -45] + list(range(165, 201, 7)) + list(range(214, 250, 7)):
